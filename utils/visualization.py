...
def show_prototypes(prototypes, patch_size, names, title='', cs=None):
    ...
    for p in prototypes:
        ...
        if cs is None:
            ...
            # img_rgb shape (H, W, 3) with float64 values, min=-0.98, max=13.33
            vmin = img_rgb.min()
            vmax = img_rgb.max()

            # normalize globally to [0,1]
            img_rgb_norm = (img_rgb - vmin) / (vmax - vmin)

            protos.append(img_rgb_norm)
        else:
    ...
    plt.tight_layout()
    plt.show()
def show_prototypes_global(prototypes, patch_size, names, title='', cs=None):
    m = int(patch_size ** 2)
    protos = []
    for p in prototypes:
        r = p[0:m].reshape((patch_size, patch_size))
