...
class IALVQ(ClassifierMixin):
    ...
    def __init__(self, prototypes_per_class=2, initial_prototypes=None, initial_omegas=None, omega_rank=None,
    ...
    def predict(self, x, ret_dist=False, return_projected = False):
        ...
        X = self._compute_distance(x)
        if ret_dist:
            if return_projected:
                return self.c_w_[dist.argmin(1)], dist, X
            else:
                return self.c_w_[dist.argmin(1)], dist
        else:
            if return_projected:
                return self.c_w_[dist.argmin(1)], X
            else:
    ...
    def _optimize(self):
        variables = np.append(self.w_, np.concatenate(self.omegas_),
                              axis=0).ravel()  # First rows are prototypes, following are omegas
