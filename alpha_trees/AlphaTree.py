...
class AlphaTree:
    ...
    def __init__(self, img, patch_sz):
        ...
        self.nodes = [Node(patch_sz=patch_sz, channels=6) for _ in range(2 * img_sz)]
        self.roots = [0] * 2 * img_sz
        self.edgeq = EdgeQueue()
        self.current_sz = img_sz

    def new_node(self, alpha):
        self.nodes[self.current_sz] = Node(alpha=alpha, parent=BOTTOM, patch_sz=self.patch_sz, channels=6)
  ...
  or does not have a parent.
    ...
    def build(self, alphas, model, labels=None, alpha_start=0):
        if labels is None:
            alphas_h, alphas_v, self.patches, self.labels = differences_per_pixel_global(model, self.img, self.patch_sz, channels=6)
        else:
            alphas_h, alphas_v, self.patches = differences_per_pixel(model, self.img, self.patch_sz, labels, channels=6)
